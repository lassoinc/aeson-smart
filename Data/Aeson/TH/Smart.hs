...
deriveJSON :: (String -> String)
           ...
           -> Bool
           -- ^ Whether to require JSON instances for type variables
           -> Name
           -- ^ Name of the type for which to generate 'ToJSON' and 'FromJSON'
           -- instances.
           -> Q [Dec]
deriveJSON withField b name =
    liftM2 (++)
           (deriveToJSON   withField b name)
           (deriveFromJSON withField b name)


deriveToJSON :: (String -> String)
             -- ^ Function to change field names.
             -> Bool
             -- ^ Whether to require ToJSON instances for type variables
             -> Name
...
deriveToJSON withField b name =
  ...
  where
    ...
    fromCons tvbs cons =
        instanceD
          (if b then return (map (\t -> ClassP ''ToJSON [VarT t]) typeNames) else return [])
...
deriveFromJSON :: (String -> String)
               -- ^ Function to change field names.
               -> Bool 
               -- ^ Whether to require FromJSON instances for type variables
               -> Name
...
deriveFromJSON withField b name =
  ...
  where
    fromCons :: [TyVarBndr] -> [Con] -> Q Dec
    fromCons tvbs cons =
        instanceD (if b then return (map (\t -> ClassP ''FromJSON [VarT t]) typeNames) else return [])
                  (classType `appT` instanceType)
                  [ funD 'parseJSON
                         [ clause []
...
consFromJSON tName withField cons = do
    ...
    lam1E (varP arg) $ doE [
      ...
      , noBindS $ caseE (varE matcher) ([parseCon tName withField c (varE mcon) False | c <- cons] ++ [noMatch tName])]
...
parseCon :: Name -> (String -> String) -> Con -> ExpQ -> Bool -> Q Match
parseCon _ _ (NormalC conName []) mcon _ = do
  str <- newName "str"
  grd <- normalG $ conEq (varE str) conName
  expr <- conE conName
  match (conP 'String [varP str]) (guardedB $ [tupSeq (normalG $ conEq (varE str) conName, [|ignore|] `appE` mcon `appE` ([|return|] `appE` conE conName))]) []
parseCon _ _ (NormalC conName [_]) mcon _ = do
  arg <- newName "arg"
  match (varP arg) (guardedB [tupSeq (conGuard mcon conName, 
    infixApp (conE conName) [e|(<$>)|] ([e|parseJSON|] `appE` varE arg))]) []
parseCon tName _  (NormalC conName ts) mcon _ = parseProduct tName conName (genericLength ts) mcon
parseCon tName withField (RecC conName ts) mcon b = do
    obj <- newName "recObj"
    let (x:xs) = [do
                    b' <- if b then return False else isInstance ''Default [ty]
                    [|lookupField|]
...
parseCon tName _ (InfixC _ conName _) mcon _ = parseProduct tName conName 2 mcon

parseCon tName withField (ForallC _ _ con) mcon _ = parseCon tName withField con mcon True
-- parseCon tName withField (ForallC _ _ con) mcon = parseCon tName withField con mcon
-- THIS THROWS A GHC PANIC!!!
